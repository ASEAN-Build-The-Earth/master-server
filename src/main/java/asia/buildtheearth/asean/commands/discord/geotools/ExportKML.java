...
import static asia.buildtheearth.asean.commands.discord.GeoToolsCommand.KML_LABEL;
...
public class ExportKML extends CommandWithModal<Interaction> implements SubCommand {

    private final ExportCommand parent;

    public ExportKML(ExportCommand parent) {
        super(parent);
        this.parent = parent;
    }
    ...
    public MainGuildCommand.Config getCommandConfig() {
        return MainGuildCommand.Config.GEO_TOOLS_EXPORT_KML;
    }
    ...
    protected Task<Interaction> acceptModal(@NotNull ReceivedEvent<DiscordModalInteractionEvent> session) {
        ...
        return parent.verify(KML_LABEL, execution, session, session.getEvent().asJDA().getMessage());
    }
