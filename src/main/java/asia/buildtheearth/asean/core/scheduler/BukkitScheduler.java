...
import asia.buildtheearth.asean.utils.TaskUtil;
import asia.buildtheearth.asean.utils.function.CheckedRunnable;
...
public class BukkitScheduler extends StandardScheduler implements ServerScheduler {
    ...
    public BukkitScheduler(MasterServer plugin) {
        this.plugin = plugin;
    }
...
//        if (!discordSRV.isReady() && discordSRV.status() == DiscordSRV.Status.SHUTTING_DOWN) {
//            discordSRV.bootstrap().mainThreadTasksForDisable().add(task);
//            return;
//        }

        runNormal.accept(plugin.getServer(), plugin);
    }
